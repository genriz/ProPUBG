...
import app.propubg.main.menu.adapters.ResultsSearchAdapter
import app.propubg.main.menu.model.MenuViewModel
import app.propubg.main.menu.model.resultsOfTournament
import com.bumptech.glide.Glide
import org.json.JSONObject

class FragmentMenuResults:Fragment(), ResultsAdapter.OnClick, ResultsSearchAdapter.OnClickListener {

    private lateinit var binding: FragmentMenuResultsBinding
    private val viewModel: MenuViewModel by activityViewModels()
    private lateinit var adapter: ResultsAdapter
    private lateinit var adapterSearch: ResultsSearchAdapter
        ...
        viewModel.realmReady.observe(viewLifecycleOwner,{
            it?.let { ready ->
                if (ready){
                    ...
                    adapterSearch = ResultsSearchAdapter(this)
                    binding.recyclerResults.setHasFixedSize(true)
                    binding.recyclerResults.adapter = adapter
                    binding.recyclerResultsSearch.adapter = adapterSearch
                    binding.recyclerResultsSearch.isVisible = false
        ...
        viewModel.searchString.observe(viewLifecycleOwner,{
            it?.let{ searchString ->
                if (searchString.length>1){
                    ...
                    binding.recyclerResultsSearch.isVisible = true
                    adapterSearch.submitList(viewModel.searchResultsLocal(searchString))
                    binding.recyclerResultsSearch.postDelayed({

...
class FragmentTournamentsUpcoming: Fragment(), TournamentsAdapter.OnClick,
        ...
        viewModel.realmReady.observe(viewLifecycleOwner,{
            it?.let{ ready ->
                if (ready){
                    ...
                    adapter = TournamentsAdapter(viewModel.getTournamentsUpcoming(), this)
        ...
        viewModel.searchString.observe(viewLifecycleOwner,{
            it?.let{ searchString ->
                if (searchString.length>1){
                    ...
                    adapterSearch.submitList(viewModel.searchTournamentsUpcomingLocal(searchString))
    ...
    override fun onTournamentClick(tournament: tournament) {
        (activity as MainActivity).openTournamentDetails(tournament, "UpcomingDetails")
    }

    override fun onResume() {
        super.onResume()
        val json = JSONObject()
        json.put("Screen", "Tournaments[Upcoming]")
